...
func TestGetChannelByID(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Found",
			setup:     setupFoundChannel,
			wantFound: true,
		},
		{
			name:      "Not Found",
			setup:     setupNotFoundChannel,
			wantFound: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			runGetChannelByIDTest(t, tc.setup, tc.wantFound)
		})
	}
}

func setupFoundChannel(t *testing.T) (*repository.Queries, int64, func()) {
	db := initializeTestDBWithChannels(t)
	q := repository.New(db)
	user := createTestUser(t, q)
	...
	ch, err := q.CreateChannel(context.Background(), repository.CreateChannelParams{
		Name:        "testchannel",
		Description: sql.NullString{String: "Test channel", Valid: true},
		CreatedBy:   user.ID,
	})
	if err != nil {
		t.Fatalf(msgCreateChannelFailed, err)
	...
	return q, ch.ID, func() { _ = db.Close() }
}

func setupNotFoundChannel(t *testing.T) (*repository.Queries, int64, func()) {
	db := initializeTestDBWithChannels(t)
	return repository.New(db), 9999, func() { _ = db.Close() }
}

func runGetChannelByIDTest(t *testing.T, setup func(t *testing.T) (*repository.Queries, int64, func()), wantFound bool) {
	q, id, cleanup := setup(t)
	defer cleanup()

	got, err := q.GetChannelByID(context.Background(), id)

	if !wantFound {
		validateChannelNotFound(t, err)
		return
	}

	validateChannelFound(t, got, err, id)
}

func validateChannelNotFound(t *testing.T, err error) {
	if err == nil {
		t.Fatal("expected error for non-existent ID, got nil")
	}
}

func validateChannelFound(t *testing.T, got repository.Channel, err error, expectedID int64) {
	if err != nil {
		t.Fatalf(msgGetChannelByIDFailed, err)
	}
	if got.ID != expectedID {
		t.Fatalf("expected ID %d, got %d", expectedID, got.ID)
	}
}
